  ...
  while (std::getline(inFile, line))
  {
    ...
    if (it != lemma_list.end())
    {
      ...
      if (dejotated_word == lemma.lemma_form)
      {
        ...
        outFile << lemma.lemma_form << '|' << lemma.conj_type << '\n';
      }
      else
      {
        outFile << dejotated_word << '|' << lemma.conj_type << '\n';
      }
    }
...
int main()
{
  ...
  std::string filename3 = str_Truncate(filename, 4) + "_autoreconstructed_just_infex_class.csv";
