...
int form_col_no(std::string filename)
{
  ...
  while (row[j].find(form) == -1)
  {
    ...
    if (j == num_of_columns)
    {
      std::cout << "No 'form' header was found\n";
      j = 999;
      break;
    }
  }
...
int lemma_id_col_no(std::string filename)
{
  ...
  while (row[j].find(lemma_id) == -1)
  {
    ...
    if (j == num_of_columns)
    {
      std::cout << "No 'lemma_id' header was found\n";
      j = 999;
      break;
    }
  }
...
int morphology_col_no(std::string filename)
{
  ...
  while (row[j].find(morphology) == -1)
  {
    ...
    if (j == num_of_columns)
    {
      std::cout << "No 'morphology' header was found\n";
      j = 999;
      break;
    }
  }
...
void createBasefiles(std::string filename2, int form_pos, int lemma_id_pos, int morphology_pos)
{
  ...
  while (std::getline(inFile, line))
  {
    ...
    outFile << clean_Cyr(cyr_form) << "|" << glagoliticise(clean_Cyr(cyr_form)) << '\n';
    outFile2 << deep_clean_Cyr(cyr_form) << "|" << lemma_id << "|" << morph_tag << "|\n";
  ...
  while (std::getline(inFile, line))
  {
    ...
    if (it != lemma_list.end())
    {
      ...
      if (dejotated_word == lemma.lemma_form)
      {
        ...
        outFile << lemma.lemma_id << '|' << line2 << '|' << lemma.lemma_form << '|' << Dejotate(lemma.morph_replace) << '|' << '|' << lemma.poss_doublet << '|' << lemma.loan_place << '|' << lemma.long_adj << '|' << boolToStr(lemma.non_assim) << '|' << lemma.eng_trans << '|' << '|' << lemma.etym_disc << '|' << lemma.bad_etym << '|' << lemma.loan_source << '|' << '|' << '|' << lemma.conj_type << '|' << '|' << '|' << '\n';
      }
      else
      {
        outFile << lemma.lemma_id << '|' << line2 << '|' << dejotated_word << '|' << Dejotate(lemma.morph_replace) << '|' << '|' << lemma.poss_doublet << '|' << lemma.loan_place << '|' << lemma.long_adj << '|' << boolToStr(lemma.non_assim) << '|' << lemma.eng_trans << '|' << lemma.lemma_form << '|' << lemma.etym_disc << '|' << lemma.bad_etym << '|' << lemma.loan_source << '|' << '|' << '|' << lemma.conj_type << '|' << '|' << '|' << '\n';
      }
    }
